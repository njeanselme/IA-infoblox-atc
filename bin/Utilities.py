...
class KennyLoggins:
    ...
    def get_logger(self, app_name=None, file_name="kenny_loggins", log_level=logging.INFO):
        ...
        formatter = logging.Formatter(
            '%(asctime)s log_level=%(levelname)s: pid=%(process)d tid=%(threadName)s file=%(filename)s:%(funcName)s:%(lineno)d message="%(message)s"')
    ...
    def get_credential(self, realm, cuser):
        ...
        try:
            ...
            self._log.error('message="{}" exception_type="{}" '
                            'exception_arguments="{}" filename="{}" line="{}" '
                            'session_key={}'.format(str(e), type(e).__name__, e, fname, exc_tb.tb_lineno, self._session_key))
            raise Exception(json.dumps(jsondump))
